...
namespace ReactNativeWasm::Components {
  ...
  void View::mountChildComponentView(const Component &childComponentView, uint32_t index) noexcept {
  //   m_panel.Children().InsertAt(index, static_cast<const BaseComponentView &>(childComponentView).Element());
  }

  void View::unmountChildComponentView(const Component &childComponentView, uint32_t index) noexcept {
