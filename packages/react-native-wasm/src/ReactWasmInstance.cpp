...
void Instance::setupRuntime(
  std::shared_ptr<facebook::react::ModuleRegistry> moduleRegistry,
  std::shared_ptr<std::vector<std::shared_ptr<facebook::react::NativeModule>>> modules) {
...
std::string nativeCallSyncHook(unsigned int moduleId, unsigned int methodId, std::string jsonArgs) {
  std::cout << "nativeCallSyncHook:: " << std::to_string(moduleId) << "::" << std::to_string(methodId) << "==>"
            << jsonArgs << std::endl;
  ...
  if (!module) {
    std::cout << "nativeCallSyncHook::Error "
              << "Module id " + std::to_string(moduleId) + " doesn't exist" << std::endl;
  ...
  emscripten::class_<facebook::react::NativeModule>("NativeModule")
    ...
    .function("getConstants", emscripten::optional_override([](facebook::react::NativeModule &self) {
                std::string constants(
                  std::make_unique<facebook::react::JSBigStdString>(folly::toJson(self.getConstants()))->c_str());
    ...
    .function(
      ...
      emscripten::optional_override(
        [](facebook::react::NativeModule &self, std::string methodName, std::string jsonArgs) {
          auto args = folly::parseJson(jsonArgs);
            ...
            if (it->name == methodName) {
              ...
              if (result.hasValue()) {
                std::string resultAsJson(
                  std::make_unique<facebook::react::JSBigStdString>(folly::toJson(result.value()))->c_str());
